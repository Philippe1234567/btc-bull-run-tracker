...
# ---------- 5. Flux ETF (Google Sheets automatisé) ----------
st.subheader("5. 🟥 Flux net ETF (Google Sheets)")
try:
    df_etf = pd.read_csv("https://docs.google.com/spreadsheets/d/1bJuWRqEYw3YM3kU0hZWfXAWTi5FGftswBwrY89pA1ew/export?format=csv")
    if "Net Flow USD" in df_etf.columns:
        etf_net_flow = df_etf["Net Flow USD"].dropna().sum()
    else:
        etf_net_flow = 0
    st.metric("Flux net ETF", f"${etf_net_flow/1e6:.1f}M")
    etf_signal = etf_net_flow > 0
except Exception:
    etf_net_flow = 50000000
    st.warning("⚠️ Impossible de lire Google Sheets — estimation utilisée")
