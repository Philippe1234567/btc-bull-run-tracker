...
# ---------- 5. Flux ETF depuis Farside ----------
st.subheader("5. 🟥 Flux net ETF (automatisé)")
try:
    url = "https://www.farside.co.uk/2023/etf-flows"
    tables = pd.read_html(url)
    df = tables[0]
    df.columns = [col.lower().strip() for col in df.columns]
    net_cols = [col for col in df.columns if "net" in col and "flow" in col]
    if net_cols:
        col = net_cols[0]
        etf_net_flow = df[col].dropna().sum()
    else:
        etf_net_flow = 0
    ...
    st.warning("⚠️ Échec de récupération Farside — estimation utilisée")
